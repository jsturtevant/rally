...
### 2026-04-03 — CI Fix: package-lock.json Out of Sync (PR #439)

- **Problem:** All 6 CI test jobs failed on PR #439 with `npm ci` EUSAGE error — `package-lock.json` was out of sync with `package.json`. Missing `acorn@8.16.0` and `@types/node@25.5.2` from the lockfile.
- **Root cause:** The batch dependency update was done with npm 11 (Node 24) which resolves optional dependency chains differently than npm 10 (Node 20/22 used in CI). Specifically, npm 11 didn't include `acorn` as a top-level `node_modules` entry even though `import-in-the-middle` depends on it. CI's npm 10 expected it to be there.
- **Fix attempt 1:** Ran `npm install` — added `@types/node` but didn't add `acorn` (npm 11 still skipped it).
- **Fix attempt 2:** Deleted `node_modules` and `package-lock.json`, ran fresh `npm install`. This time npm correctly resolved and included `acorn@8.16.0`. All 114 tests pass locally. All 7 CI checks green.
- **Lesson:** When upgrading deps, always do a clean `rm -rf node_modules && rm package-lock.json && npm install` to ensure the lockfile is fully consistent. Incremental `npm install` on a lockfile generated by a different npm major version can miss transitive deps. Also: CI npm version (10.x) != local npm version (11.x) — be aware of resolution differences.

### 2026-04-03 — Session Logged (CI Fix for PR #439)

- Scribe recorded orchestration log: `.squad/orchestration-log/2026-04-03T20-16-kaylee.md`
- Scribe recorded session log: `.squad/log/2026-04-03-ci-fix.md`
- No decisions in inbox for this session
- All 7 CI checks now passing on PR #439
