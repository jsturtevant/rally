...
| `waiting`      | 🟣   | waiting             | Marked as waiting (manual status via `p` key)    |
...
3. **waiting** — Manual status set via dashboard `p` key after human review.
...
3. **waiting** — Manual status set via dashboard `p` key.
...
Dispatches already in `reviewing` or `waiting` are not affected by the automatic refresh.
