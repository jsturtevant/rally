...
export function launchCopilot(worktreePath, prompt, opts = {}) {
  ...
  try {
    ...
    if (opts.sandbox) {
      ...
      // The sandbox mounts the worktree as the working directory so Copilot
      // can read and modify files while keeping the host system safe.
      cmd = 'docker';
      args = [
        'sandbox', 'run',
        '--workdir', worktreePath,
        'gh', 'copilot', '-p', `workspace ${prompt}`,
      ];
    } else {
      cmd = 'gh';
