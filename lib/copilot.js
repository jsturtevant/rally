 ...
 *
 * When opts.sandbox is true, runs Copilot inside a Docker sandbox microVM using
 * `docker sandbox run copilot <workspace> -- [agent-args]` syntax. The same
 * deny-tool flags and read-only policy are passed after the -- separator.
 * Requires GH_TOKEN or GITHUB_TOKEN set globally in shell config and Docker
 * Desktop restarted so the daemon picks up the credential.
...
export function launchCopilot(worktreePath, prompt, opts = {}) {
  ...
  try {
    ...
    if (opts.sandbox) {
      ...
      // Agent args (deny-tool flags and prompt) go after the -- separator.
      cmd = 'docker';
      args = [
        'sandbox', 'run',
