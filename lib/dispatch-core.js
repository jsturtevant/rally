...
import { writeCopilotInstructions } from './copilot-instructions.js';
...
export function setupDispatchWorktree(opts) {
  ...
  let copilotResult = { sessionId: null, process: null, logPath: null };
  try {
    ...
    // Write read-only copilot-instructions.md into worktree
    writeCopilotInstructions(worktreePath);

    // Launch Copilot CLI (if available), optionally inside a Docker sandbox
    if (sandbox && checkDockerSandboxAvailable({ _exec })) {
      try {
        const logPath = join(worktreePath, '.copilot-output.log');
        copilotResult = launchCopilot(worktreePath, copilotPrompt, { _spawn, logPath, sandbox: true });
      } catch {
        // Copilot sandbox launch failed — continue without it
      }
    } else if (!sandbox && checkCopilotAvailable({ _exec })) {
      try {
        const logPath = join(worktreePath, '.copilot-output.log');
        copilotResult = launchCopilot(worktreePath, copilotPrompt, { _spawn, logPath });
      } catch {
