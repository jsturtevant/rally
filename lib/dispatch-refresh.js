 ...
 * Dispatches in "planning" or "implementing" whose PID has exited get moved to "done".
 *
 * @param {object} [opts]
...
export function refreshDispatchStatuses(opts = {}) {
  ...
  for (const d of dispatches) {
    if (d.status !== 'planning' && d.status !== 'implementing' && d.status !== 'reviewing') {
      continue;
    }
