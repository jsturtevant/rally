...
export async function dispatchRemove(number, opts = {}) {
  ...
  try {
    ...
    const { extractionResult } = await cleanupDispatch(dispatch, repoPath, {
      _terminatePid: opts._terminatePid,
      _removeWorktree: opts._removeWorktree,
      _exec: opts._exec,
      _extractLearnings: opts._extractLearnings,
      _chalk,
      silent: opts.silent, // suppress console output when called from TUI
    });
    ...
    if (!opts.silent) {
      ...
      console.log(`${_chalk.green('✓ Removed')} ${typeLabel} #${number} (branch ${_chalk.dim(dispatch.branch)} deleted)`);
    }
    return { dispatch, extractionResult };
  } catch (err) {
    if (!opts.silent) {
      console.error(`${_chalk.red('✗ Failed')} to remove #${number}: ${err.message}`);
    }
    throw err;
  }
}
