...
/**
 * Get trust warnings for an item WITHOUT prompting.
 * Returns an array of warning objects (empty if no warnings).
 *
 * @param {object} opts
 ...
 * @returns {Array<{type: string, message: string, detail: string}>}
 */
export function getTrustWarnings({ type, number, repo, _exec = execFileSync }) {
  ...
  if (author.toLowerCase() !== currentUser.toLowerCase()) {
    warnings.push({
      ...
      message: `This ${type} was created by ${author}, not you (${currentUser}).`,
      detail: 'Content from other users may contain prompt injection attacks.',
    });
  }
  ...
  if (isMember === false) {
    warnings.push({
      type: 'org_membership',
      message: `You (${currentUser}) are not a member of the ${repo.split('/')[0]} org.`,
      detail: 'You may be dispatching untrusted content.',
    });
  }
  return warnings;
}
 ...
 * @param {object} [opts._chalk] - Injectable chalk instance (for testing)
...
export async function checkDispatchTrust(opts) {
  ...
  if (author.toLowerCase() !== currentUser.toLowerCase()) {
    warnings.push(
      `${_chalk.yellow('⚠ Author mismatch:')} This ${type} was created by ${_chalk.bold(author)}, not you (${_chalk.bold(currentUser)}).`,
  ...
  if (isMember === false) {
    warnings.push(
      `${_chalk.yellow('⚠ Org membership:')} You (${_chalk.bold(currentUser)}) are not a member of the ${_chalk.bold(repo.split('/')[0])} org.`,
