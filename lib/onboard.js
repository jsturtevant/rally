...
export function configureForkRemotes(projectPath, fork, _exec, _chalk = chalk) {
  ...
  exec(['fetch', 'origin']);
  console.log(_chalk.green('✓') + ` Fetched from origin (${forkInfo.owner}/${forkInfo.repo})`);
 ...
 */
function registerProject({ projectPath, gitDir, teamDir, teamType, fullRepoName, forkInfo, _chalk }) {
  ...
  if (existing) {
    ...
    console.log(_chalk.green('✓') + ` Registered project: ${projectName} (${teamType} team)`);
  }
}
...
export async function onboard(options = {}) {
  ...
  // 5. Select team
  const { teamDir, teamType } = await selectTeam({
    team: options.team,
    _exec: options._exec,
  ...
  registerProject({ projectPath, gitDir, teamDir, teamType, fullRepoName, forkInfo, _chalk });
}
