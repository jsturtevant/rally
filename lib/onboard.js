 ...
 * @param {Function} [options._ensurePersonalSquad] - Injectable ensurePersonalSquad function (for testing)
...
export async function onboard(options = {}) {
  ...
  let forkInfo = null;
  if (options.fork) {
    // Auto-discovery: fork === 'auto' means derive fork from GitHub username + repo name
    const needsAutoDiscovery = forkWithoutPath || options.fork === 'auto';
    if (needsAutoDiscovery) {
      // Derive user's fork from GitHub username
      ...
      // Get repo name from parsed URL or from git remote
      const repoName = parsed ? parsed.repo : (() => {
        ...
        if (!info) throw new RallyError('Cannot determine repo name for fork auto-discovery.', EXIT_CONFIG);
        return info.repo;
      })();
      forkInfo = configureForkRemotes(projectPath, `${username}/${repoName}`, options._exec, _chalk);
    } else {
      forkInfo = configureForkRemotes(projectPath, options.fork, options._exec, _chalk);
    }
  }

  // 5. Ensure personal squad exists (prompt to create if missing)
  // _ensurePersonalSquad is injected by bin/rally.js; defaults to pass-through for library use
  const _ensureSquad = options._ensurePersonalSquad || (async () => true);
  const squadReady = await _ensureSquad();
  if (!squadReady) {
    throw new RallyError('A personal squad is required to onboard projects.', EXIT_CONFIG);
  }

  // 6. Get personal squad directory
  const { teamDir } = await _selectTeam();

  // 7. Create symlinks
  setupSymlinks(projectPath, teamDir, _chalk);

  // 8. Register project and manage excludes
