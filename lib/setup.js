...
import { getCopilotInstructions } from './copilot-instructions.js';
...
export async function setup(options = {}) {
  ...
  // Write dispatch-policy.md into squad directory
  const policyPath = join(squadDir, 'dispatch-policy.md');
  if (existsSync(squadDir) && !existsSync(policyPath)) {
    writeFileSync(policyPath, getCopilotInstructions(), 'utf8');
    console.log(chalk.green('✓') + ` Created dispatch policy at ${policyPath}`);
  }
