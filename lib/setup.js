...
export async function setup(options = {}) {
  ...
  if (existsSync(teamDir)) {
    ...
    console.log(_chalk.green('✓') + ` Created team directory at ${teamDir}`);
  }
  ...
  if (existsSync(projectsDir)) {
    ...
    console.log(_chalk.green('✓') + ` Created projects directory at ${projectsDir}`);
  }
  ...
  writeConfig(config);
  console.log(_chalk.green('✓') + ` Saved config to ${join(configDir, 'config.yaml')}`);
}
