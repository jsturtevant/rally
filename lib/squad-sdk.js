...
export async function ensurePersonalSquad(options = {}) {
  ...
  // Non-TTY (piped stdin, CI): auto-create with defaults — no prompts possible
  if (!process.stdin.isTTY) {
    console.log(_chalk.yellow('No personal squad found — creating with defaults (non-interactive)...'));
    const result = await initPersonalSquad({ _initSquad: options._initSquad });
    if (result.success) {
      console.log(_chalk.green('✓') + ' Personal squad created.');
    }
