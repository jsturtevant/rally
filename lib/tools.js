  ...
  for (const tool of toolsToCheck) {
    try {
      if (opts._exec) {
        ...
        // execFileSync avoids shell injection. On Windows, CLI tools like npx
        // are .cmd files that execFileSync can't resolve directly — retry with
        // .cmd extension if the direct call fails with ENOENT.
        try {
          execFileSync(tool, ['--version'], { stdio: 'pipe' });
        } catch (err) {
          if (process.platform === 'win32' && err.code === 'ENOENT') {
            execFileSync(`${tool}.cmd`, ['--version'], { stdio: 'pipe' });
          } else {
            throw err;
          }
        }
