...
function SummaryLine({ summary }) {
  return /* @__PURE__ */ React.createElement(Box, { marginTop: 1 }, /* @__PURE__ */ React.createElement(Text, null, /* @__PURE__ */ React.createElement(Text, { bold: true, color: "green" }, summary.active, " active"), /* @__PURE__ */ React.createElement(Text, null, " \xB7 "), /* @__PURE__ */ React.createElement(Text, { bold: true, color: "blue" }, summary.done, " done"), /* @__PURE__ */ React.createElement(Text, null, " \xB7 "), /* @__PURE__ */ React.createElement(Text, { bold: true, color: "red" }, summary.orphaned, " orphaned")));
}
function Dashboard({ project, onSelect, onAttachSession, onDispatchItem, onDispatch, getTrustWarnings: getTrustWarningsProp, onAddProject, refreshInterval = 5e3, _spawn = defaultSpawn, _dispatchRemove = defaultDispatchRemove, _parseSessionIdFromLog = defaultParseSessionId, _updateDispatchStatus = defaultUpdateDispatchStatus, _listOnboardedRepos, _fetchIssues, _fetchPrs }) {
  const { exit } = useApp();
  ...
  const hasConnectableSession = actionDispatch?.session_id && UUID_RE.test(actionDispatch.session_id);
  ...
  function removeSelectedDispatch(dispatch) {
    _dispatchRemove(dispatch.number, { repo: dispatch.repo }).then(() => setRefreshKey((k) => k + 1)).catch((err) => {
  ...
  if (detailViewDispatch) {
    return /* @__PURE__ */ React.createElement(Box, { flexDirection: "column", height: stdout.rows }, /* @__PURE__ */ React.createElement(
      DetailView,
      {
  ...
  if (dispatchStatus && dispatchPending) {
    return /* @__PURE__ */ React.createElement(Box, { flexDirection: "column", height: stdout.rows }, /* @__PURE__ */ React.createElement(DispatchStatus, { item: dispatchPending, status: dispatchStatus, message: dispatchMessage }));
  }
  ...
  const dispatchRows = data.dispatches.length + 1;
  const contentUsed = dispatchRows + 2 + 3;
  const availableContent = stdout.rows ? Math.max(0, stdout.rows - 4) : 20;
  const padCount = Math.max(0, availableContent - contentUsed);
  return /* @__PURE__ */ React.createElement(Box, { flexDirection: "column", borderStyle: "round", borderColor: "gray", paddingX: 1 }, /* @__PURE__ */ React.createElement(Box, { marginBottom: 1 }, /* @__PURE__ */ React.createElement(Text, { bold: true }, "\u{1F680} Rally Dashboard")), /* @__PURE__ */ React.createElement(DispatchTable, { dispatches: data.dispatches, selectedIndex }), /* @__PURE__ */ React.createElement(SummaryLine, { summary: data.summary }), Array.from({ length: padCount }, (_, i) => /* @__PURE__ */ React.createElement(Text, { key: `pad-${i}` }, " ")), /* @__PURE__ */ React.createElement(Box, { marginTop: 1, flexDirection: "column", alignItems: "center" }, /* @__PURE__ */ React.createElement(Text, { dimColor: true }, "\u2191/\u2193 navigate \xB7 Enter actions \xB7 d details \xB7 l logs \xB7 v open \xB7 o browser \xB7 c connect IDE"), /* @__PURE__ */ React.createElement(Text, { dimColor: true }, "n new dispatch \xB7 a attach \xB7 p pushed \xB7 x delete \xB7 r refresh \xB7 q quit")));
}
export {
  computeSummary,
