...
export function renderPlainDashboard({ project } = {}) {
  ...
  const colWidths = { type: 7, issueRef: 30, branch: 28, folder: 30, status: 20, changes: 10, age: 6 };
  const header = [
  ...
  if (dispatches.length === 0) {
    ...
    for (let gi = 0; gi < groups.length; gi++) {
      ...
      for (const d of group.dispatches) {
        ...
        let issueRef = d.type === 'branch' ? '' : `#${d.number}`;
        if (d.title) {
          if (d.type === 'branch') {
            issueRef = d.title.length > colWidths.issueRef
              ? d.title.slice(0, colWidths.issueRef - 1) + '…'
              : d.title;
          } else {
