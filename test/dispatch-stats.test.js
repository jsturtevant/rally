...
describe('enrichWithStats', () => {
  ...
  test('enrichWithStats enriches dispatch with stats from log file for done status', () => {
    ...
    const dispatches = [{ id: 'd1', status: 'waiting', logPath }];
  ...
  test('does not enrich when logPath is missing', () => {
    const dispatches = [{ id: 'd1', status: 'waiting' }];
    const result = enrichWithStats(dispatches);
  ...
  test('does not enrich when log file does not exist', () => {
    const dispatches = [{ id: 'd1', status: 'waiting', logPath: '/nonexistent/log.txt' }];
    const result = enrichWithStats(dispatches);
  ...
  test('handles multiple dispatches correctly', () => {
    ...
    const dispatches = [
      { id: 'd1', status: 'waiting', logPath: logPath1 },
